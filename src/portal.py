...
class NvidiaWebPortal:
    ...
    async def list_downloads(self) -> Optional[List[MetaInfo]]:
        ...
        async with self._session.post(url, json=data) as resp:
            if resp.status == 200:
                ...
                utils.log_error_and_raise(_logger, f"List downloads request failed with status {resp.status}")
                return None
    ...
    async def get_download_url(self, download_id: str) -> Optional[DownloadInfo]:
        ...
        async with self._session.post(url, json=data) as resp:
            if resp.status == 200:
                ...
                utils.log_error_and_raise(_logger, f"Get download url request failed with status {resp.status}")
                return None
    ...
    async def _update_userinfo(self):
        ...
        async with self._session.get(userinfo_url) as resp:
            if resp.status == 200:
                info = await resp.json()
                orgname = info["user"]["orgName"]
                async with self._session.get(vgroups_url_tmpl.format(orgname=orgname)) as v_resp:
                    ...
                    utils.log_error_and_raise(_logger, f"Get virtual groups request failed with status {v_resp.status}")
            utils.log_error_and_raise(_logger, f"Update userinfo request failed with status {resp.status}")
    ...
    async def _wait_for_verification_link(self, timeout=60) -> str:
        ...
        while time.time() < start_time + timeout:
            ...
            mail_data_list = [await self._gmail_client.get_mail(eid, mailbox) for mailbox, eid in eid_tuples]
    ...
    async def load_session_from_cookies(self):
        ...
        if self._https_proxy:
            connector = aiohttp.ProxyConnector(proxy=self._https_proxy)
