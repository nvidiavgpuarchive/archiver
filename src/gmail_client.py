...
class GmailClient():
    ...
    async def connect(self):
        ...
        if not self._mailboxes:
            ...
            for mailbox in mailboxes:
                ...
                attributes = []
                for attr in mailbox.split('"/"')[0].strip(' ').lstrip('(').rstrip(')').split(' '):
    ...
    async def get_mail(self, eid, mailbox="INBOX"):
        ...
        if status != "OK":
            utils.log_error_and_raise(_logger, f"Cannot fetch email {eid} from {mailbox}")

        msg = email.message_from_bytes(msg_data[1])
        # Subject
        subj_raw, subj_enc = decode_header(msg.get('Subject'))[0]
        subject = subj_raw.decode(subj_enc or 'utf-8', errors='replace') if isinstance(subj_raw, bytes) else (
                subj_raw or '')
        # Sender
        from_raw, from_enc = decode_header(msg.get('From'))[0]
        sender = from_raw.decode(from_enc or 'utf-8', errors='replace') if isinstance(from_raw, bytes) else (
                from_raw or '')
        # Recipients
