...
class IAClient:
        ...
        if not multipart:
            ...
            for fp in filepaths:
                if os.path.getsize(fp) < 1024 ** 2 * 8:
                    ...
                    await self.mulitpart_upload_file(bucket, fp, headers)

        _logger.info(f"Uploaded {len(filepaths)} files to internet archive.")

    @staticmethod
    async def _file_chunker(path, chunk_size=1024 ** 2):
    ...
    async def _multipart_init(self, bucket: str, filename: str, headers: dict[str, Any]):
        ...
        async with aiohttp.ClientSession() as session:
            async with self._connection_semaphore:
                async with session.post(url, headers=headers, proxy=self._proxy) as resp:
                    ...
                    return tree.findtext('UploadId')

    @retry(stop=stop_after_attempt(3), wait=wait_fixed(3))
    async def _multipart_upload_part(self, bucket: str, filename: str, upload_id: str, part_number: int, data):
        url = f"https://s3.us.archive.org/{bucket}/{filename}?uploadId={upload_id}&partNumber={part_number}"
        async with aiohttp.ClientSession() as session:
            async with self._connection_semaphore:
                async with session.put(url, data=data, headers=self._default_headers, proxy=self._proxy) as resp:
                    if resp.status >= 400:
                        raise Exception("Bad status code.")
                    return resp.headers.get('ETag')

    @retry(stop=stop_after_attempt(3), wait=wait_fixed(3))
    async def _multipart_complete(self, bucket: str, filepath: str, upload_id: str, parts: dict[str, Any]):
        filename = os.path.basename(filepath)
        url = f"https://s3.us.archive.org/{bucket}/{filename}?uploadId={upload_id}"
        xml = "<CompleteMultipartUpload>"
        for part in parts:
            xml += f"<Part><PartNumber>{part['part_number']}</PartNumber><ETag>{part['etag']}</ETag></Part>"
        xml += "</CompleteMultipartUpload>"
    ...
    async def multipart_abort(self, bucket, filename, upload_id):
        ...
        async with aiohttp.ClientSession() as session:
            async with self._connection_semaphore:
                async with session.delete(url, proxy=self._proxy, headers=self._default_headers) as resp:
                    if resp.status >= 400:
                        raise Exception("Bad status code.")
                    return await resp.text()
    ...
    async def mulitpart_upload_file(self,
        ...
        upload_id = await self._multipart_init(bucket, filename, headers)
        ...
        try:
            ...
            async for chunk in self._file_chunker(filepath, chunk_size=chunk_size):
                etag = await self._multipart_upload_part(bucket, filename, upload_id, part_num, chunk)
                parts.append({
            ...
            except:  # there's nothing we can do about it
                pass
    ...
    async def verify_bucket(self,
        ...
        while asyncio.get_running_loop().time() - start_time < timeout:
            ...
            try:
                ...
                if result == [] or (result != [] and ("pending_tasks" not in info or info["pending_tasks"] == False)):
                    return result
            except KeyError:
...
async def main():
    ...
    dummy_path = "/tmp/dummy_1g.bin"
    chunk_size = 8 * 1024 * 1024  # 8MB
    total_size = 1024 * 1024 * 1024  # 1GB

    if not os.path.exists(dummy_path) or os.path.getsize(dummy_path) != total_size:
        print(f"Generating 1GB test file at {dummy_path}...")
        async with aiofiles.open(dummy_path, "wb") as f:
            bytes_written = 0
            data = b"\0" * chunk_size
            while bytes_written < total_size:
                write_size = min(chunk_size, total_size - bytes_written)
                await f.write(data[:write_size])
    ...
    async with IAClient(access_key=config["ia"]["s3_access_key"], secret_key=config["ia"]["s3_secret_key"], ) as client:
        ...
        bucket = "testbucket-" + "".join(random.choices(string.ascii_lowercase, k=10))
        print(f"Using bucket: {bucket}")

        # Create bucket if needed (you may need to implement client.create_bucket if not present)
        object_name = "test_1g_file.bin"

        print("Starting multipart upload...")
        await client.create_bucket(bucket=bucket,
                                   filepaths=[dummy_path],
                                   meta_mediatype="texts",
                                   meta_title="Test upload",
                                   meta_description="Test upload",
